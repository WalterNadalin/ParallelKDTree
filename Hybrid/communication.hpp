...
N* distribute(int master, int slave, int size, unsigned short &axis,
  ...
  if(rank == master) {
    ...
    if(size / 2 > 0) {
       ...
       head = distribute(rank, rank + size / 2, size / 2, axis, tree->left_ptr, data);
    }
    ...
    MPI_Recv(&size, 1, MPI_INT, MPI_ANY_SOURCE, TAG_SIZ, MPI_COMM_WORLD, &status);   
    MPI_Recv(&length, 1, MPI_UNSIGNED_LONG, MPI_ANY_SOURCE, TAG_LEN, MPI_COMM_WORLD,
             &status);    
    MPI_Recv(&axis, 1, MPI_UNSIGNED_SHORT, MPI_ANY_SOURCE, TAG_DIR, MPI_COMM_WORLD,
             &status);
    ...
    if(size / 2 > 0) {
       ...
       head = distribute(rank, rank + size / 2, size / 2, axis, tree->right_ptr, data);
    }
