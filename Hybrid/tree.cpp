...
int main(int argc, char **argv) {
  ...
  // Commented follows an example of how it is possible to move through the tree
  // using properly the functions
  /*
  if (rank == master)
    cout << "Root: " << tree << endl;
  ...
  // We know that the master (processor 0) will always have a copy of the head 
  // of the tree
  auto tmp_prc = right(tmp_ptr, master);

  if (rank == tmp_prc)
    cout << "Going to the right: " << '(' << tmp_ptr->pnt[0] << ", " << tmp_ptr->pnt[1] << ')' << endl;
